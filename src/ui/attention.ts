 ...
 *  celle qui attend dès qu'il est clos — `startRun` la montera au clic sur
 *  « Nouvelle descente », et c'est entre deux descentes qu'on fait ses courses.
 *  Les DEUX fins de descente comptent, la mort comme le boss vaincu : tester
 *  `=== 'dead'` ferait chiffrer à la Forge le châssis qu'on vient d'arrêter de
 *  piloter, pendant qu'on prépare le suivant. La Forge et `attention` doivent
 *  lire la même, sinon le point rouge et la ligne « avant → après » juste à
 *  côté se contrediraient. */
export function shoppingToupie(meta: MetaState, run: RunState): ToupieId {
  return run.phase !== 'fighting' ? meta.toupies.active : run.toupie;
}
