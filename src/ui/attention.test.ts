...
describe('attention', () => {
  ...
  it('signale le Fondateur dès qu’il est réclamable', () => {
    const meta = empty();
    // `chapterValidated` avant la progression des chapitres : le méta retient
    // désormais QUEL chapitre est validé, et le cadeau s'ouvre dès le premier.
    meta.bestChapter = 1;
...
describe('shoppingToupie', () => {
  ...
  // l'arène tant que le run vit, celle qui attend dès qu'il est clos — perdu
  // comme gagné.
  ...
  // Le boss vaincu ferme la descente au même titre que la mort. Vérifié par
  // mutation : la condition `=== 'dead'` d'avant la progression des chapitres
  // fait rougir ce test, et lui seul.
  it('bascule aussi une fois le boss vaincu', () => {
    const meta = createInitialMeta(1);
    const run = { phase: 'won', toupie: 'brasier-solaire' } as RunState;
    meta.toupies.active = 'typhon-primal';
    expect(shoppingToupie(meta, run)).toBe('typhon-primal');
  });
});
