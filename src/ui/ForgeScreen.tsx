...
import { resolveProfile } from '../sim/profile';
...
interface SlotRow {
  key: Slot;
  label: string;
  /** Abrégé de la stat portée par l'emplacement. **Les mêmes trois lettres que le
   *  radar** (`StatRadar`) : le joueur n'apprend qu'un vocabulaire, pas deux. */
  axis: string;
  read: (s: Stats) => number;
}

const SLOTS: SlotRow[] = [
  { key: 'lame', label: 'Lame', axis: 'ATQ', read: (s) => s.attack },
  { key: 'disque', label: 'Disque', axis: 'DÉF', read: (s) => s.defense },
  { key: 'pointe', label: 'Pointe', axis: 'VIT', read: (s) => s.maxSpeed },
  { key: 'noyau', label: 'Noyau', axis: 'SPIN', read: (s) => s.spinMax },
];
            ...
            >
              ...
              <span style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0, flex: '1 1 0' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--muted)', fontVariantNumeric: 'tabular-nums' }}>
                  {row.axis} {formatCredits(row.read(before))} → <span style={{ color: 'var(--text)' }}>{formatCredits(after)}</span>
                </span>
